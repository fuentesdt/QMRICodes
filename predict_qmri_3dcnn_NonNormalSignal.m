...
% - Loads Excel with acquisition parameters (choose .xlsx/.xls)
% - Loads T1w/T2w/FLAIR (+ optional mask)
% - Runs sliding-window prediction (GPU if present)
% - Saves PD/T1/T2/g1/g2/g3 as NIfTI in savedir
...
% -------------------------------------------------------------------------
% savedir defines the folder name used to store prediction results for a
% specific LOPO (Leave-One-Patient-Out) fold.
%
% Example: "P1L1OutPred"
%   → Model was trained by leaving out Patient 1 (P1)
%   → This trained model is then used to generate predictions for ALL patients
%
% For each patient, a subfolder with this name will be created inside that
% patient’s directory:
%
%   Patient_i/
%       P1L1OutPred/
...
%
% Important:
%   - For Patient 1 → these are true held-out predictions (model never saw this data)
%   - For other patients → predictions are from a model that was trained including them
%
% This structure enables consistent evaluation of:
%   (1) Held-out performance (generalization)
%   (2) In-training performance (for comparison/gap analysis)
% -------------------------------------------------------------------------
savedir = "P1L1OutPred"; % rename it when you use another LOPO network
...
net = S.net;
if isfield(S,'config'), cfgTr = S.config; else, cfgTr = struct(); end

%% 1) Select Excel with acquisition parameters
[xlsFile, xlsPath] = uigetfile({'*.xlsx;*.xls','Excel (*.xlsx, *.xls)'}, ...
    'Select Excel file with all patients acquisition parameters');
if isequal(xlsFile,0) || isequal(xlsPath,0)
    error('Excel selection cancelled.');
end
...
%% 2) Select root folder containing patient subfolders
rootDir = uigetdir(pwd, 'Select the ROOT folder that contains all patient folders');
...
%% 3) If you want to make predictions for all the patients using the trained network
for sel = 1:numel(valid)
    ...
    cd(savedir);savefolder = pwd;cd(rootDir);
    ...
    assert(isequal(size(S1),size(S2),size(S3),size(M)), 'Input/mask sizes do not match.');

    %% 5) Acquisition parameters for this patient
    row = T(strcmp(string(T.(idCol)), pid), :);
    ...
    stride = max(1, floor(patchSize/4));            % 50% overlap by default
    ...
    %%
    % Enforce physical caps (as in training)
    PDmax = pickField(cfgTr,'PDmax');
    T1max = pickField(cfgTr,'T1max');
    T2max = pickField(cfgTr,'T2max');
    g1max = pickField(cfgTr,'g1max');
    g2max = pickField(cfgTr,'g2max');
    g3max = pickField(cfgTr,'g3max');
    ...
    % Apply light 3D Gaussian smoothing (sigma = 0.1)
    sigma = 0.1;
    ...
    %% 9) Synthesize signals from predictions (no T2* in GRE)
    ACQb = reshape(ACQ, [8 1]); % (8,1)
    ...
    %% Training (active) loss in your script (UNMASKED L1)
    % ----------------- SIGNAL LOSS (masked L1) -----------------
    d1 = abs(S1 - S1calc).*M;
    d2 = abs(S2 - S2calc).*M;
    d3 = abs(S3 - S3calc).*M;

    Lsig = sum(d1,'all') * cfgTr.wSignal(1) + ...
           sum(d2,'all') * cfgTr.wSignal(2) + ...
           sum(d3,'all') * cfgTr.wSignal(3);

    % ----------------- PARAM LOSS (masked L1) ------------------
    Lpar = cfgTr.lamPD * sum(abs((PD  - PDSyMRI).*M),'all') + ...
           cfgTr.lamT1 * sum(abs((T1  - T1SyMRI).*M),'all') + ...
           cfgTr.lamT2 * sum(abs((T2  - T2SyMRI).*M),'all');
    ...
    fprintf('  Lpar = %.6g\n', Lpar);
    fprintf('  Loss = %.6g\n', TestLoss);
    fprintf('--- Training losses ---\n');
    fprintf('  Training Loss = %.6g\n', trainLoss);
    fprintf('  Validation Loss = %.6g\n', valLoss);
    
    
    save(fullfile(savefolder,'TestLosses.mat'),"Lsig","Lpar","TestLoss","trainLoss","valLoss");
    %% Plotting and correlation
    cccPD = ccc_barnes(PD(find(M==1)),PDSyMRI(find(M==1)));
    cccT1 = ccc_barnes(T1(find(M==1)),T1SyMRI(find(M==1)));
    cccT2 = ccc_barnes(T2(find(M==1)),T2SyMRI(find(M==1)));
    cccS1 = ccc_barnes(S1(find(M==1)),S1calc(find(M==1)));
    cccS2 = ccc_barnes(S2(find(M==1)),S2calc(find(M==1)));
    cccS3 = ccc_barnes(S3(find(M==1)),S3calc(find(M==1)));
    ...
    nexttile; scatter(S1_meas, S1_pred, 6, 'filled'); hold on; grid on; axis square; axis tight;
    ...
    nexttile; scatter(S2_meas, S2_pred, 6, 'filled'); hold on; grid on; axis square; axis tight;
    xlabel('T2W raw data'); ylabel('T2W model'); title(sprintf('T2W, CCC = %.2f',cccS2));
    lims = [min([xlim ylim]) max([xlim ylim])]; plot(lims,lims,'k--'); xlim(lims); ylim(lims);
    nexttile; scatter(SF_meas, SF_pred, 6, 'filled'); hold on; grid on; axis square; axis tight;
    ...
    % figure('Name','Observed vs Predicted Q-maps','Color','w');
    % tiledlayout(1,3,'Padding','compact','TileSpacing','compact');
    nexttile; scatter(PDobs, PDpred, 6, 'filled'); hold on; grid on; axis square; axis tight;
    xlabel('PD SyMRI'); ylabel('PD Pred'); title(sprintf('PD, CCC = %.2f',cccPD));
    lims = [min([xlim ylim]) max([xlim ylim])]; plot(lims,lims,'k--'); xlim(lims); ylim(lims);
    nexttile; scatter(T1obs, T1pred, 6, 'filled'); hold on; grid on; axis square; axis tight;
    xlabel('T1 SyMRI'); ylabel('T1 Pred'); title(sprintf('T1, CCC = %.2f',cccT1));
    lims = [min([xlim ylim]) max([xlim ylim])]; plot(lims,lims,'k--'); xlim(lims); ylim(lims);
    nexttile; scatter(T2obs, T2pred, 6, 'filled'); hold on; grid on; axis square; axis tight;
    ...
    fprintf(sprintf('PID:%s Prediction Done. ✅\n',pid));
    % end % ===== main =====
end % For loop over patients in the folder
...
function out = pickField(S, field)
if isempty(S) || ~isfield(S,field), disp("Upper bounds for predictions are not defined"); else, out = S.(field); end
end

function tf = canUseGPU()
tf = false;
try, tf = gpuDeviceCount>0; catch, end
end

function s = ternary(cond,a,b), if cond, s=a; else, s=b; end, end

function V = readNii(fp)
V = niftiread(fp); V = single(V);
end
...
V = single(V);
try, info.Datatype = 'single'; catch, end
% Write .nii or .nii.gz
...
for i=1:3
    v = S{i}; vi = v(M);
    if isempty(vi), vi = v(:); end
    p1 = prctile(vi,1); p99 = prctile(vi,99);
    v = min(max(v,p1), p99);
    mu = mean(v(M),'omitnan'); if isnan(mu), mu = mean(v(:),'omitnan'); end
    sd = std(v(M),1,'omitnan'); if sd<=0 || isnan(sd), sd = std(v(:),1,'omitnan'); end
...
TRT1 = ACQ(1); FAT1 = ACQ(3); TRT2 = ACQ(4); TET2 = ACQ(5);
TRF  = ACQ(6); TEF  = ACQ(7); TIF  = ACQ(8);
...
% --- spatial sizes ---
Zdim = size(X,1);  Ydim = size(X,2);  Xdim = size(X,3);
...
Zp = size(Xpad,1);  Yp = size(Xpad,2);  Xp = size(Xpad,3);

% --- Gaussian blending window (separable) ---
% Try gausswin (Signal Processing Toolbox); fall back to exp if unavailable.
try
    alpha = 3;
    wz = gausswin(ps(1), alpha); wy = gausswin(ps(2), alpha); wx = gausswin(ps(3), alpha);
    % disp('GWinUsed')
catch
    % Fallback: continuous Gaussian with sigma covering ~1/6 of patch
    sz = max(ps(1), 1); sy = max(ps(2), 1); sx = max(ps(3), 1);
    zc = (0:ps(1)-1) - (ps(1)-1)/2; yc = (0:ps(2)-1) - (ps(2)-1)/2; xc = (0:ps(3)-1) - (ps(3)-1)/2;
    wz = exp(-(zc.^2)/(2*sz^2)).';  wy = exp(-(yc.^2)/(2*sy^2)).';  wx = exp(-(xc.^2)/(2*sx^2)).';
    % disp('ManUsed')
end
win = reshape(wz, [ps(1) 1 1]) .* reshape(wy, [1 ps(2) 1]) .* reshape(wx, [1 1 ps(3)]);
win = single(win ./ max(win(:)));   % normalize to [0,1], cast to single
...
for z = 1:st(1):Zp - ps(1) + 1
    for y = 1:st(2):Yp - ps(2) + 1
        for x = 1:st(3):Xp - ps(3) + 1
            patch = Xpad(z:z+ps(1)-1, y:y+ps(2)-1, x:x+ps(3)-1, :);  % [pz py px 3]

            % dlarray as [Z Y X C B]
            dlX = dlarray(reshape(patch, [ps(1) ps(2) ps(3) 3 1]), 'SSSCB');
            if useGPU, dlX = gpuArray(dlX); end

            % predict one patch
            Ypatch = predict(net, dlX);                  % [pz py px 6 1]
            Ypatch = gather(extractdata(Ypatch));        % to CPU single

            % weighted accumulation (Gaussian window)
            for c = 1:6
                A(z:z+ps(1)-1, y:y+ps(2)-1, x:x+ps(3)-1, c) = ...
                    A(z:z+ps(1)-1, y:y+ps(2)-1, x:x+ps(3)-1, c) + Ypatch(:,:,:,c) .* win;
            end
