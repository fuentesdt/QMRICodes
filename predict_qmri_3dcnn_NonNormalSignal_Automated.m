...
tokens = regexp(modelFolder, 'trained_models_P(\d+)', 'tokens');

if strcmpi(modelFolder, 'trained_models_all')
    % Model trained using all patients
    modelMode  = "all";
    foldNumber = NaN;
    leftOutID  = "";
    savedir    = "All_Pred";
...
elseif ~isempty(tokens)
    % LOPO model: trained leaving out patient P<foldNumber>
    modelMode  = "lopo";
    foldNumber = str2double(tokens{1}{1});

    if foldNumber < 1 || foldNumber > numel(patientIDs)
        error('Fold number P%d exceeds number of patients in Excel file.', foldNumber);
    end

    leftOutID = patientIDs(foldNumber);
    savedir   = sprintf('P%dL1OutPred', foldNumber);

else
    error(['Model folder name must be either trained_models_all ', ...
        'or trained_models_P1, trained_models_P2, etc.']);
end
...
for sel = 1:numel(predictIDs)
    ...
    if modelMode == "all"
        ...
        save(fullfile(savefolder,"TestLosses.mat"), ...
            "Lsig","Lpar","TestLoss","trainLoss","valLoss", ...
            "modelMode","foldNumber","leftOutID","pid","savedir");
    end
