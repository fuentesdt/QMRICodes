...
config.outDir      = fullfile(config.rootDir,'trained_models_All');
config.idCol       = "PatientID";
...
% Saving
% config.outDir      = fullfile(config.rootDir,"trained_models_nn4l");
config.ckptEvery   = 20;
...
for k = 1:numel(patients)
    ...
    % Normalize inputs (clip p1–p99, z-score in mask)
    [S1,S2,S3] = deal(S1,S2,S3);
...
idx   = 1;  % cursor
    ...
    function [X, refs, M, A, S, sScale, edgeW] = next()
        ...
        % Per-contrast robust scales (p95) for loss normalization
        sScale = single([1 1 1]);
...
%%
% % Signal-space Huber (normalized by per-contrast sScale)
% s1 = sScale(1); s2 = sScale(2); s3 = sScale(3);
% Lsig = huber((S1 - Smeas(:,:,:,1,:))/s1,1.0,M)*cfg.wSignal(1) + ...
%     huber((S2 - Smeas(:,:,:,2,:))/s2,1.0,M)*cfg.wSignal(2) + ...
%     huber((S3 - Smeas(:,:,:,3,:))/s3,1.0,M)*cfg.wSignal(3)
%
% % Parameter-space (masked): PD L1; log-space L1 for T1/T2
% Lpar = cfg.lamPD * meanMasked(abs(PD - refs.PD), M) + ...
%     cfg.lamT1 * meanMasked(abs(log(T1 + 1e-8) - log(refs.T1 + 1e-8)), M) + ...
%     cfg.lamT2 * meanMasked(abs(log(T2 + 1e-8) - log(refs.T2 + 1e-8)), M)

% Lsig = sum(abs(S1 - Smeas(:,:,:,1,:)),'all')*cfg.wSignal(1) + ...
%     sum(abs(S2 - Smeas(:,:,:,2,:)),'all')*cfg.wSignal(2) + ...
%     sum(abs(S3 - Smeas(:,:,:,3,:)),'all')*cfg.wSignal(3);
% 
% Lpar = cfg.lamPD *sum(abs(PD - refs.PD),'all') + ...
%     cfg.lamT1 * sum(abs(T1  - refs.T1), 'all')+ ...
%     cfg.lamT2 * sum(abs(T2  - refs.T2),'all' );
% % Regularization
